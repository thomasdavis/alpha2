#!/usr/bin/env bash
# Finite semantic-chat continuation from the selected public checkpoint.
#
# Usage:
#   scripts/run_chat_semantic_repair_v4.sh \
#     TRAIN DEV CORPUS_MANIFEST EVAL_FREEZE TOKENIZER INIT_CHECKPOINT RUN_DIR \
#     [STEPS=800] [LR=0.00002] [LR_MIN=0.000002]
...
init_checkpoint=${6:?selected native checkpoint required}
...
[[ -z $(git status --porcelain) ]] || { echo "v4 training requires a clean committed worktree" >&2; exit 1; }
...
const contract = {
  schema: "alpha-chat-semantic-repair-contract-v4",
  purpose: "test whether a compact, reviewed semantic-chat curriculum improves meaning while preserving natural conversation",
  ...
  intervention: {
    changed: "positive data distribution: reviewed semantic chat plus limited natural multi-turn replay",
    unchanged: ["architecture", "tokenizer", "chat template", "loss implementation", "answer-start weighting", "EOS weighting", "decoding"],
    excluded: ["SODA bulk replay", "RCR unlikelihood", "sealed-final tuning"],
  },
  training: {
    steps: Number(process.env.STEPS), blockSize: 512, batchSize: 16,
    gradientAccumulationSteps: 1, optimizer: "AdamW reset from selected checkpoint parameters",
    learningRate: Number(process.env.LR), learningRateMin: Number(process.env.LR_MIN),
    warmupSteps: 50, checkpointInterval: 100, deterministicEpochShuffle: true,
    equalConversationWeight: true, answerStartTokens: 4, answerStartMultiplier: 8,
    answerEndMultiplier: 2, rcrUlWeight: 0, fp16: false,
  },
  selection: {
    checkpoints: [100, 200, 300, 400, 500, 600, 700, 800].filter((step) => step <= Number(process.env.STEPS)),
    rule: "select only on held-out free-generation meaning and conversational behavior; validation loss cannot select",
...
exec nice -n 5 ionice -c2 -n7 node --expose-gc apps/cli/dist/main.js train \
  ...
  --steps="$steps" --lr="$learning_rate" --lrMin="$learning_rate_min" --warmupIters=50 \
  --beta1=0.9 --beta2=0.95 --eps=0.00000001 --weightDecay=0.1 --gradClip=1.0 \
  --spikeThreshold=50 --evalInterval=100 --checkpointInterval=100 --evalIters=10 \
