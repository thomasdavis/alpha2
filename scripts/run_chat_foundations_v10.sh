#!/usr/bin/env bash
# Short, early-checkpoint chat repair over the independently reviewed v10 corpus.
...
steps=${12:-200}
learning_rate=${13:-0.000005}
learning_rate_min=${14:-0.0000005}
...
[[ -z $(git status --porcelain) ]] || { echo "v10 training requires a clean committed worktree" >&2; exit 1; }
...
assert(corpus.rows?.train >= 9000 && corpus.rows?.dev >= 400, "v10 corpus unexpectedly small");
assert(corpus.invariants?.allTrainingRowsIndependentlyReviewed === true, "unreviewed data can train");
assert(corpus.invariants?.originalDevelopmentPopulationByteExact === true, "fixed development population drifted");
assert(corpus.invariants?.newWaveDevelopmentExcluded === true, "new-wave data leaked into development");
...
assert(rcr.schema === "alpha-chat-foundations-v10-rcr-ul-remap-v1", "unexpected RCR-UL remap schema");
...
const contract = {
  schema: "alpha-chat-foundations-contract-v10",
  purpose: "test whether a second independent reviewed curriculum wave broadens robust ordinary conversation",
  ...
  intervention: {
    changed: "roughly doubled unique GPT-5.4 conversational situations with independent GPT-5.5 review",
    unchanged: ["U1 parameters", "architecture", "tokenizer", "chat template", "decoding", "original v8 development population", "frozen evaluation"],
    retained: ["U1-derived RCR-UL repetition trajectories at weight 0.5"],
    excluded: ["public-output replay", "generic bulk SFT", "programming curriculum", "sealed-final tuning", "exact BLAH prompt training"],
  },
  training: {
    steps,
    blockSize: 512,
    ...
    warmupSteps: Math.min(25, Math.floor(steps / 4)),
  ...
  selection: {
    checkpoints: Array.from({ length: Math.floor(steps / 50) }, (_, index) => (index + 1) * 50),
    rule: "matched greedy semantics plus repeated temperature-0.7/top-40 robustness; neither loss nor isolated nonempty output can select",
    matchedPublicBaselineRequired: true,
    matchedU1EvidenceRequired: true,
    matchedV8Step200Required: true,
...
((warmup_steps > 25)) && warmup_steps=25
