...
- **Pretrain**: 1B tokens minimum (17 tokens/param — demo-grade but honest; stretch 2-3B if G2 lands
  ...
  **LR SWEEP LIVE 2026-07-23:** functional/source commit `e6d9430` rebuilt 19/19 on the RTX 3090 pod
  ...
  pinned source/data/tokenizer contract. Through its step-4,500 held-out gate all rows are consecutive
  and finite, train/held-out loss is 3.6362/3.8100, median post-step-100 throughput is 3,840 tok/s,
  and all 46 allocator samples report exactly 34 slabs with zero overflow. The remote/mounted
  4,500-row prefix is byte-identical at `df586b4a…`; checkpoint 4,000 remains a hash-mirrored,
  native-audited 692,528,815-byte ALPH file at `c728fc85…`, with every parameter finite/nonzero.
  The guard safely retained exactly 2,000/3,000/4,000 after verified pruning. Training resumed through
  step 4,550 at 3,909 tok/s. Post-save telemetry stayed bounded at 4,038MB
  RSS / 3,148MB external with zero swap, so the third save added no further RSS step. Current origin
  still fixes the retained snapshot and will be NVIDIA-proven before flagship. Its aligned held-out
  result is 0.1083 worse than `1e-3`, an interim signal.
