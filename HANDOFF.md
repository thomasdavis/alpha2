# HANDOFF — alpha2 revival, state as of 2026-07-24 ~01:58 UTC
...
- **Second contracted LR pilot (`2e-3`) is LIVE** at
  ...
  deliberately pinned `e6d9430` source/data/tokenizer contract. Through its step-5,000 checkpoint gate,
  all rows are consecutive/finite, train/held-out loss is 3.9816/3.7241, median post-step-100
  throughput is 3,842 tok/s, and all 51 allocator samples report exactly 34 slabs with zero overflow.
  The remote/mounted 5,000-row prefix is byte-identical at SHA-256 `ca08944f…`; checkpoint 5,000 is
  a hash-mirrored/native-audited 692,528,815-byte ALPH file at `79690f9e…`, with all 57,688,576
  parameters finite/nonzero. The guard safely retained exactly 3,000/4,000/5,000 after verified pruning.
  Training resumed through step 5,050 at 3,899 tok/s. Post-save telemetry
  stayed bounded at 4,040MB RSS / 3,148MB external with zero swap, so the latest save added no further
  RSS step after the increase observed at checkpoint 2,000. Current origin still fixes the retained
  snapshot and will be NVIDIA-proven before flagship. At the aligned gate `2e-3` is 0.0971 held-out loss
...
- RunPod balance was **$58.0194897522** at about 01:57 UTC; total account burn was $0.301/hr including
